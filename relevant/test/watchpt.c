...
struct rb_control_frame_struct {
	rb_execution_context_t *ec; // Pointer to the execution context
};
...
int main() {
	...
	rb_execution_context_t *ec = (rb_execution_context_t *)malloc(sizeof(rb_execution_context_t));
	ruby_current_ec = ec;

	ruby_current_ec->raised_flag = 0;

	VALUE ecc = (VALUE)ruby_current_ec; // Set the execution context pointer

	ecc |= (VALUE)1 << 75; 

	ruby_current_ec = (rb_execution_context_t *)ecc; // Cast back to rb_execution_context_t pointer


	rb_control_frame_t *cfp = (rb_control_frame_t *)malloc(sizeof(rb_control_frame_t));
	cfp->ec = ruby_current_ec; // Set the execution context pointer
